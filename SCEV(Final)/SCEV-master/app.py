...
def profileDb(name,info):
    ...
    try:
        ...
        data = {}
    data[name] = info
    # j_path = os.path.join(m_path, 'static\\data\\phone.json')
...
def gen():
    ...
    while True:
        try:
            ...
            else:reqDict = {"status":0,"name":"","mobile":"","profile":""}
...
def form():
    try:
        if request.method == 'POST':
            ...
            i_path = os.path.join('static\\images\\'+name, secure_filename(profile_files[0].filename))#secure_filename(file.filename) 
